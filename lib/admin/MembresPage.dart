...
class _MembresPageState extends State<MembresPage> {
  ...
  void dispose() {
    ...
    _adresseCtrl.dispose(); _professionCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _saveMembre([Membre? membre]) async {
    if (_nomCtrl.text.trim().isEmpty || _professionCtrl.text.trim().isEmpty) {
      _showSnack('Nom et profession obligatoires', Colors.red); return;
    }
    final data = {
    ...
    try {
      if (membre == null) {
        ...
        if (mounted) { Navigator.pop(context); _showSnack('Membre #${newMembre.numRegistre} enregistré !', Colors.green); }
      } else {
        final res = await ApiService().modifierMembre(membre.id, data);
        TontineStore().mettreAJourMembre(Membre.fromJson(res));
        if (mounted) { Navigator.pop(context); _showSnack('Membre mis à jour', Colors.green); }
      }
  ...
  Future<void> _abandonner(Membre membre) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Row(children: [
          ...
          const SizedBox(width: 10), const Text('Abandon confirmé ?'),
        ]),
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: membre.perdArgentSiAbandon ? Colors.red.shade200 : Colors.green.shade200),
            ),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(
                ...
                style: TextStyle(
                    color: membre.perdArgentSiAbandon ? Colors.red.shade700 : Colors.green.shade700,
    ...
    try {
      ...
      if (mounted) _showSnack('${membre.nom} marqué abandonné.', Colors.red.shade700);
    } on ApiException catch (e) {
  ...
  void _showForm([Membre? membre]) {
    ...
    final nextNum = (store.membres.isEmpty
        ? 0
        : store.membres.map((m) => m.numRegistre).reduce((a, b) => a > b ? a : b)) + 1;
              ...
              crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  Container(padding: const EdgeInsets.all(9),
                      ...
                      child: const Icon(Icons.person_add_rounded, color: _primary)),
                  const SizedBox(width: 12),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(membre == null ? 'Nouveau Membre' : 'Modifier Membre',
                ...
                const SizedBox(height: 12),
                _fField(_professionCtrl, 'Profession *',           Icons.work_rounded),
                const SizedBox(height: 12),
                _fField(_telCtrl,        'Téléphone (WhatsApp) *', Icons.phone, type: TextInputType.phone),
                const SizedBox(height: 12),
                _fField(_adresseCtrl,    'Adresse / Quartier',     Icons.location_on),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text(membre == null ? 'ENREGISTRER' : 'METTRE À JOUR',
      ...
      builder: (_) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 40, height: 4, margin: const EdgeInsets.only(bottom: 16),
              decoration: BoxDecoration(color: Colors.grey.shade300, borderRadius: BorderRadius.circular(10))),
          Text(membre.nom, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
          Text('#${membre.numRegistre} • ${membre.profession}',
              style: TextStyle(color: Colors.grey.shade500, fontSize: 12)),
          const Divider(height: 20),
          _aTile(Icons.account_circle_rounded, 'Voir le profil complet', _primary, () {
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (_) => MembreProfilPage(membre: membre)));
          }),
          _aTile(Icons.edit_rounded, 'Modifier', Colors.blue, () {
            Navigator.pop(context); _showForm(membre);
          }),
          if (!membre.aAbandonne)
            _aTile(
              membre.isActive ? Icons.block_rounded : Icons.check_circle_rounded,
              ...
              membre.isActive ? Colors.orange : Colors.green,
                ...
                try {
                  if (membre.isActive) {
                    await ApiService().modifierMembre(membre.id, {'is_active': false});
          ...
          if (!membre.aAbandonne)
            _aTile(Icons.exit_to_app_rounded, 'Marquer comme abandonné', Colors.red.shade700, () {
              Navigator.pop(context); _abandonner(membre);
            }),
          _aTile(Icons.delete_forever_rounded, 'Supprimer définitivement', Colors.red, () {
            Navigator.pop(context); _supprimer(membre);
          }),
        ]),
      ),
    );
        ...
        return GestureDetector(
          ...
          child: Scaffold(
            ...
            appBar: AppBar(
              ...
              actions: [
                IconButton(icon: const Icon(Icons.refresh, color: Colors.white), onPressed: store.chargerMembres),
              ],
            ),
            ...
            body: Column(children: [
              Container(
                ...
                child: Column(children: [
                  Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                    _hStat(nbActifs.toString(),          'Actifs',   Colors.greenAccent),
                    _divH(),
                    _hStat(nbInactifs.toString(),        'Inactifs', Colors.white70),
                    _divH(),
                    _hStat(nbAbandons.toString(),        'Abandons', Colors.redAccent.shade100),
                    _divH(),
                    _hStat(store.membres.length.toString(), 'Total', Colors.white),
                  ]),
                  const SizedBox(height: 12),
                  Container(
                    ...
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      Text('Total cotisé : ${fmt.format(totalCotise)} CFA',
              ...
              Container(
                ...
                child: Column(children: [
                  Padding(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        filled: true, fillColor: const Color(0xFFF1F3F4), contentPadding: EdgeInsets.zero,
                  ...
                  SingleChildScrollView(
                    ...
                    child: Row(children: ['Tous', 'Actifs', 'Inactifs', 'Abandonnés'].map((type) {
                      ...
                      return Padding(
                        ...
                        child: ChoiceChip(
                          label: Text(type), selected: sel,
                          onSelected: (_) => setState(() => _filterType = type),
                          selectedColor: chipColor.withOpacity(0.15), backgroundColor: Colors.grey.shade100,
                          side: BorderSide(color: sel ? chipColor : Colors.grey.shade200),
              ...
              Expanded(
                child: store.isLoading
                  ...
                  const SizedBox(height: 12),
                  Text(store.errorMsg!, style: TextStyle(color: Colors.grey.shade500)),
                  const SizedBox(height: 12),
                  ElevatedButton(onPressed: store.chargerMembres, child: const Text('Réessayer')),
                ]))
                    : _filtered.isEmpty
                    ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
  ...
  Widget _divH() => Container(height: 28, width: 1, color: Colors.white.withOpacity(0.2));

  Widget _aTile(IconData icon, String title, Color color, VoidCallback onTap) =>
      ListTile(leading: Icon(icon, color: color),
          title: Text(title, style: TextStyle(color: color, fontWeight: FontWeight.w600)),
          onTap: onTap);

  Widget _fField(TextEditingController ctrl, String label, IconData icon, {TextInputType type = TextInputType.text}) {
    return TextField(
  ...
  Widget _buildCard(Membre membre) {
    ...
    final borderColor = estAbandon ? Colors.red.shade300
        : membre.isActive ? Colors.green.withOpacity(0.3) : Colors.grey.withOpacity(0.3);
    ...
    return Container(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(18),
        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => MembreProfilPage(membre: membre))),
        child: Padding(
            ...
            Row(children: [
              Stack(children: [
                Container(
                  width: 52, height: 52,
                  decoration: BoxDecoration(
                      gradient: LinearGradient(colors: estAbandon
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Expanded(child: Text(membre.nom,
                      style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15))),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    decoration: BoxDecoration(color: membre.statutColor.withOpacity(0.12), borderRadius: BorderRadius.circular(10)),
                    child: Text(membre.statutLabel,
                        style: TextStyle(color: membre.statutColor, fontSize: 9, fontWeight: FontWeight.bold)),
                  ),
                ]),
                const SizedBox(height: 2),
                Text('${membre.profession} • ${membre.adresse}',
                    style: TextStyle(color: Colors.grey.shade500, fontSize: 11),
                    maxLines: 1, overflow: TextOverflow.ellipsis),
                const SizedBox(height: 2),
                Row(children: [
                  Icon(Icons.event_rounded, size: 10, color: Colors.blue.shade300),
                  const SizedBox(width: 3),
                  Text('Inscrit le ${membre.dateInscription}',
                      style: TextStyle(color: Colors.blue.shade400, fontSize: 9, fontWeight: FontWeight.w500)),
                ]),
              ])),
              IconButton(icon: Icon(Icons.more_vert, color: Colors.grey.shade400), onPressed: () => _showActions(membre)),
            ]),
            const SizedBox(height: 10),
            Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ClipRRect(borderRadius: BorderRadius.circular(6),
                    child: LinearProgressIndicator(
                      ...
                      valueColor: AlwaysStoppedAnimation<Color>(
                          estAbandon ? Colors.red.shade400 : membre.isActive ? Colors.green : Colors.grey),
                      minHeight: 5,
                    )),
                const SizedBox(height: 4),
                Text('${membre.semainesCotisees}/$kTotalSemaines sem.  •  ${fmt.format(membre.totalCotiseCfa)} CFA',
              ...
              if (membre.estEligibleMoto && !estAbandon)
                Container(
                  ...
                  child: const Row(children: [
                    Icon(Icons.two_wheeler_rounded, color: Color(0xFFE65100), size: 12),
                    SizedBox(width: 3),
                    Text('Moto ✓', style: TextStyle(color: Color(0xFFE65100), fontSize: 9, fontWeight: FontWeight.bold)),
                  ]),
                ),
            ]),
            if (estAbandon && membre.perdArgentSiAbandon)
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text(
                    'A cotisé ${fmt.format(membre.totalCotiseCfa)} CFA < ${fmt.format(kSeuilAbandonnement)} CFA → perd son argent',
...
class _MembreProfilPageState extends State<MembreProfilPage> {
  ...
  Future<void> _charger() async {
    ...
    try { _data = await ApiService().getMembre(widget.membre.id); } catch (_) {}
    setState(() => _isLoading = false);
  }

  // ── Données depuis l'API ───────────────────────────────────────
  // L'API /membres/{id} retourne les cotisations avec le statut
  // Filtre : payées uniquement pour affichage dans la section cotisations payées
  // Toutes pour l'historique complet

  List<Map<String, dynamic>> get _toutesLesCotisations =>
      ((_data?['membre']?['cotisations'] ?? _data?['cotisations']) as List? ?? [])
          .cast<Map<String, dynamic>>();

  /// Cotisations PAYÉES uniquement (pour affichage et calcul PDF)
  List<Map<String, dynamic>> get _cotisationsPayees =>
      _toutesLesCotisations.where((c) =>
      c['statut'] == 'paye' || c['paye'] == true).toList();

  /// Cotisations IMPAYÉES
  List<Map<String, dynamic>> get _cotisationsImpayes =>
      _toutesLesCotisations.where((c) =>
      c['statut'] == 'impaye' || c['paye'] == false).toList();

  List<Map<String, dynamic>> get _distributions =>
      ((_data?['membre']?['distributions'] ?? _data?['distributions']) as List? ?? [])
          .cast<Map<String, dynamic>>();

  List<Map<String, dynamic>> get _sanctions =>
      ((_data?['membre']?['sanctions'] ?? _data?['sanctions']) as List? ?? [])
          .cast<Map<String, dynamic>>();

  /// Total réel basé sur les lignes payées uniquement
  int get _totalCotisPaye =>
      _cotisationsPayees.fold(0, (s, c) => s + (c['montant'] as int? ?? kMontantHebdo));
  ...
  Future<File> _genererPDF({String type = 'complet'}) async {
    ...
    // Calculs corrects : seulement les semaines payées
    ...
    doc.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        ...
        pw.Container(
          ...
          child: pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
            pw.Container(
              ...
              child: pw.Center(child: pw.Text(membre.nom[0],
                  style: pw.TextStyle(color: PdfColors.white, fontSize: 26, fontWeight: pw.FontWeight.bold))),
            ),
            pw.SizedBox(width: 16),
            pw.Expanded(child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
              pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                pw.Text(membre.nom, style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                pw.Container(
                  padding: const pw.EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: pw.BoxDecoration(
                    color: membre.isActive ? PdfColors.green100 : PdfColors.red100,
                    borderRadius: pw.BorderRadius.circular(6),
                  ),
                  child: pw.Text(membre.statutLabel,
                      style: pw.TextStyle(
                          fontSize: 10, fontWeight: pw.FontWeight.bold,
                          color: membre.isActive ? PdfColors.green800 : PdfColors.red800)),
                ),
              ]),
          ...
          child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
            ...
            if (membre.aAbandonne) ...[
              ...
              pw.Container(
                ...
                decoration: pw.BoxDecoration(color: PdfColors.red100, borderRadius: pw.BorderRadius.circular(6)),
                child: pw.Text(
                  membre.perdArgentSiAbandon
        ...
        // ─ Cotisations PAYÉES ─────────────────────────────────────
        if ((type == 'complet' || type == 'cotisations') && _cotisationsPayees.isNotEmpty) ...[
          pw.SizedBox(height: 16),
          _pdfSectionTitle(
            'COTISATIONS PAYÉES ($nbSemPayees sem. × ${fmt.format(kMontantHebdo)} CFA)',
            PdfColors.teal700,
          ),
          pw.SizedBox(height: 6),
          _pdfTable(
            headers: ['Semaine', 'Date samedi', 'Montant'],
            rows: _cotisationsPayees.map((c) => [
              'Sem. ${c['num_semaine'] ?? '—'}',
              c['date_samedi']?.toString() ?? '—',
              '${fmt.format(c['montant'] ?? kMontantHebdo)} CFA',
            ]).toList(),
          ),
          pw.SizedBox(height: 6),
          _pdfTotalRow(
        ...
        // ─ Semaines IMPAYÉES (aperçu) ─────────────────────────────
        if ((type == 'complet' || type == 'cotisations') && _cotisationsImpayes.isNotEmpty) ...[
          pw.SizedBox(height: 14),
          _pdfSectionTitle(
          ...
          _pdfTable(
            ...
            rows: _cotisationsImpayes.map((c) => [
              'Sem. ${c['num_semaine'] ?? '—'}',
              c['date_samedi']?.toString() ?? '—',
              'Non encaissée',
            ]).toList(),
          ...
          _pdfTotalRow(
            'MONTANT NON PERÇU',
            '${fmt.format(nbSemImpayes * kMontantHebdo)} CFA attendus',
            PdfColors.red800, PdfColors.red50, PdfColors.red200,
          ),
        ],

        // ─ Distributions ──────────────────────────────────────────
        if ((type == 'complet' || type == 'distributions') && _distributions.isNotEmpty) ...[
          pw.SizedBox(height: 16),
          _pdfSectionTitle('DISTRIBUTIONS', PdfColors.green700),
          pw.SizedBox(height: 6),
          _pdfTable(
            headers: ['Date', 'Note', 'Montant'],
            rows: _distributions.map((d) => [
          ...
          pw.SizedBox(height: 6),
          _pdfTotalRow('TOTAL DISTRIBUTIONS', '${fmt.format(totalDistrib)} CFA', PdfColors.green800, PdfColors.green50, PdfColors.green200),
        ],

        // ─ Sanctions ──────────────────────────────────────────────
        if ((type == 'complet' || type == 'sanctions') && _sanctions.isNotEmpty) ...[
          pw.SizedBox(height: 16),
          ...
          pw.SizedBox(height: 6),
          _pdfTotalRow('TOTAL SANCTIONS', '${fmt.format(totalSanctions)} CFA', PdfColors.red800, PdfColors.red50, PdfColors.red200),
        ],
      ],
    ));
  ...
  pw.Widget _pdfBox(String val, String label, PdfColor color) => pw.Expanded(
    child: pw.Container(
      ...
      child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        pw.Text(val, style: pw.TextStyle(color: PdfColors.white, fontSize: 10, fontWeight: pw.FontWeight.bold)),
        pw.Text(label, style: const pw.TextStyle(color: PdfColors.white, fontSize: 7)),
      ]),
    ),
  );

  pw.Widget _pdfSectionTitle(String title, PdfColor color) => pw.Container(
    padding: const pw.EdgeInsets.symmetric(horizontal: 12, vertical: 7),
    decoration: pw.BoxDecoration(color: color, borderRadius: pw.BorderRadius.circular(6)),
    child: pw.Text(title, style: pw.TextStyle(color: PdfColors.white, fontSize: 11, fontWeight: pw.FontWeight.bold)),
  );

  pw.Widget _pdfTable({required List<String> headers, required List<List<String>> rows, int? statusColIndex}) {
    return pw.Table(
        ...
        ...rows.asMap().entries.map((entry) {
          ...
          return pw.TableRow(
            ...
            children: row.asMap().entries.map((e) {
              final isStatus = statusColIndex != null && e.key == statusColIndex;
              ...
              return pw.Padding(
                padding: const pw.EdgeInsets.all(5),
                child: pw.Text(e.value, style: pw.TextStyle(fontSize: 8, color: tc,
  ...
  pw.Widget _pdfTotalRow(String label, String value, PdfColor textColor, PdfColor bg, PdfColor border) =>
      pw.Container(
        padding: const pw.EdgeInsets.all(8),
        decoration: pw.BoxDecoration(color: bg, borderRadius: pw.BorderRadius.circular(6),
            border: pw.Border.all(color: border)),
        child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
          pw.Text(label, style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: textColor)),
  ...
  Future<void> _exporterEtPartager({required String type, required bool viaWhatsApp}) async {
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Row(children: [
            Container(
              ...
              decoration: BoxDecoration(color: const Color(0xFFE65100).withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
              child: const Icon(Icons.picture_as_pdf_rounded, color: Color(0xFFE65100), size: 24),
            ),
            const SizedBox(width: 14),
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Exporter PDF', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              Text('${widget.membre.nom} — #${widget.membre.numRegistre}',
                  style: TextStyle(color: Colors.grey.shade500, fontSize: 11)),
            ]),
          ]),
          const SizedBox(height: 20),
          _exportTile('Dossier complet', Icons.folder_rounded,
              'Cotisations (payées + impayées) + Distributions + Sanctions', Colors.blue, () => _lancerExport('complet')),
          _exportTile('Cotisations uniquement', Icons.monetization_on_rounded,
              'Payées et non encaissées + totaux', Colors.teal, () => _lancerExport('cotisations')),
          _exportTile('Distributions uniquement', Icons.account_balance_wallet_rounded,
              'Montants reçus', Colors.green, () => _lancerExport('distributions')),
          _exportTile('Sanctions uniquement', Icons.gavel_rounded,
              'Amendes et pénalités', Colors.red, () => _lancerExport('sanctions')),
        ]),
      ),
    );
  }

  Widget _exportTile(String title, IconData icon, String subtitle, Color color, VoidCallback onTap) {
    return Container(
      ...
      child: ListTile(
        ...
        subtitle: Text(subtitle, style: TextStyle(color: Colors.grey.shade500, fontSize: 10)),
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(width: double.infinity, height: 52,
            child: ElevatedButton.icon(
              style: ElevatedButton.styleFrom(backgroundColor: _whatsapp,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
              onPressed: _isGeneratingPdf ? null : () async {
                Navigator.pop(context);
                await _exporterEtPartager(type: type, viaWhatsApp: true);
              },
          ...
          SizedBox(width: double.infinity, height: 48,
            child: OutlinedButton.icon(
              ...
              onPressed: _isGeneratingPdf ? null : () async {
                Navigator.pop(context);
                await _exporterEtPartager(type: type, viaWhatsApp: false);
              },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(slivers: [
        SliverAppBar(
          expandedHeight: 260, pinned: true, backgroundColor: _primary,
          ...
          actions: [
            _isGeneratingPdf
                ...
                : IconButton(icon: const Icon(Icons.picture_as_pdf_rounded, color: Colors.white), onPressed: _showExportOptions),
          ],
                ...
                child: Column(children: [
                  Row(children: [
                    Container(width: 72, height: 72,
                        ...
                        child: Center(child: Text(m.nom[0],
                            style: const TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.bold)))),
                    const SizedBox(width: 16),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(m.nom, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 3),
                      Text(m.profession, style: const TextStyle(color: Colors.white70, fontSize: 12)),
                      const SizedBox(height: 4),
                      Row(children: [
                        _pill('#${m.numRegistre}', Icons.badge_rounded),
                        const SizedBox(width: 6),
                        _pill(m.statutLabel, Icons.circle, color: m.statutColor),
                      ]),
                    ])),
                  ]),
                  const SizedBox(height: 14),
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    _pill('Inscrit le ${m.dateInscription}', Icons.event_rounded),
                    _pill('Tél: ${m.telephone}', Icons.phone),
                  ]),
                  const SizedBox(height: 12),
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    Text('${m.semainesCotisees}/$kTotalSemaines semaines',
                        style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
                    Text('${fmt.format(m.totalCotiseCfa)} CFA cotisés',
        ...
        SliverToBoxAdapter(
          child: _isLoading
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              if (m.aAbandonne)
                _alertCard(Icons.exit_to_app_rounded, Colors.red.shade700, 'Membre abandonné',
              ...
              if (m.estEligibleMoto && !m.aAbandonne)
                _alertCard(Icons.two_wheeler_rounded, const Color(0xFFE65100), 'Éligible au complément moto',
                    '${m.semainesCotisees} semaines cotisées (≥ $kSeuilEligibilite). Ce membre peut faire une demande.'),
              const SizedBox(height: 16),

              // ── Cotisations payées ─────────────────────────────
              _sectionHeader('Cotisations payées (${_cotisationsPayees.length})',
                  Icons.check_circle_rounded, Colors.teal),
              const SizedBox(height: 8),
              _exportRow('cotisations', Colors.teal),
              const SizedBox(height: 8),
              if (_cotisationsPayees.isEmpty) _emptyCard('Aucune cotisation payée')
              else ..._cotisationsPayees.map((c) => _simpleCard(
                  Icons.calendar_view_week_rounded, const Color(0xFF6A1B9A),
                  'Semaine ${c['num_semaine']} — ${c['date_samedi'] ?? ''}',
                  'Payé ✓', Colors.green,
                  '${fmt.format(c['montant'] ?? kMontantHebdo)} CFA', Colors.green)),
              if (_cotisationsPayees.isNotEmpty)
                _totalRow('Total payé', fmt.format(_totalCotisPaye), Colors.teal),

              const SizedBox(height: 16),

              // ── Semaines impayées ──────────────────────────────
              _sectionHeader('Semaines non encaissées (${_cotisationsImpayes.length})',
                  Icons.radio_button_unchecked, Colors.red),
              const SizedBox(height: 8),
              if (_cotisationsImpayes.isEmpty)
                _emptyCard('Toutes les semaines sont encaissées ✓')
              else ..._cotisationsImpayes.map((c) => _simpleCard(
                  Icons.calendar_view_week_rounded, Colors.red.shade300,
                  'Semaine ${c['num_semaine']} — ${c['date_samedi'] ?? ''}',
                  'Non encaissée', Colors.red.shade400,
                  '0 CFA', Colors.red.shade400)),
              if (_cotisationsImpayes.isNotEmpty)
                _totalRow('Montant non perçu',
                    fmt.format(_cotisationsImpayes.length * kMontantHebdo), Colors.red),

              const SizedBox(height: 20),

              // ── Distributions ──────────────────────────────────
              _sectionHeader('Distributions', Icons.account_balance_wallet_rounded, Colors.green),
              const SizedBox(height: 8),
              _exportRow('distributions', Colors.green),
              const SizedBox(height: 8),
              if (_distributions.isEmpty) _emptyCard('Aucune distribution')
              else ..._distributions.map((d) => _simpleCard(
                  Icons.outbox_rounded, Colors.green,
                  d['note'] ?? 'Distribution',
                  'Le ${d['date_distribution'] ?? ''}',
                  Colors.grey.shade500,
                  '${fmt.format(d['montant'] ?? 0)} CFA', Colors.green.shade700)),
              if (_distributions.isNotEmpty)
                _totalRow('Total reçu',
                    fmt.format(_distributions.fold(0, (s, d) => s + (d['montant'] as int? ?? 0))), Colors.green),

              const SizedBox(height: 20),

              // ── Sanctions ──────────────────────────────────────
              _sectionHeader('Sanctions', Icons.gavel_rounded, Colors.red),
              const SizedBox(height: 8),
              _exportRow('sanctions', Colors.red),
              const SizedBox(height: 8),
              if (_sanctions.isEmpty) _emptyCard('Aucune sanction')
              else ..._sanctions.map((s) {
                final isPaid = s['statut'] == 'paye';
                final motif  = (s['motif'] as String? ?? '').replaceAll('_', ' ');
                return _simpleCard(Icons.gavel_rounded,
                    isPaid ? Colors.green : Colors.red,
                    motif,
                    '${s['date_sanction'] ?? ''} • ${isPaid ? 'Payé' : 'En attente'}',
                    Colors.grey.shade500,
                    '${fmt.format(s['montant'] ?? 0)} CFA', Colors.red);
              }),
              if (_sanctions.isNotEmpty)
                _totalRow('Total sanctions',
                    fmt.format(_sanctions.fold(0, (s, s2) => s + (s2['montant'] as int? ?? 0))), Colors.red),
      ...
      floatingActionButton: Column(mainAxisSize: MainAxisSize.min, children: [
        FloatingActionButton(
          heroTag: 'wa', backgroundColor: _whatsapp, mini: true,
          onPressed: _showExportOptions,
          child: const Icon(Icons.send_rounded, color: Colors.white, size: 18),
        ),
        const SizedBox(height: 8),
        FloatingActionButton.extended(
          heroTag: 'pdf', backgroundColor: const Color(0xFFE65100),
          onPressed: _isGeneratingPdf ? null : _showExportOptions,
          icon: _isGeneratingPdf
              ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
              : const Icon(Icons.picture_as_pdf_rounded, color: Colors.white),
          label: Text(_isGeneratingPdf ? 'Génération...' : 'Exporter PDF',
              style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        ),
      ]),
    );
  ...
  Widget _pill(String text, IconData icon, {Color? color}) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text(text, style: TextStyle(color: color ?? Colors.white, fontSize: 10, fontWeight: FontWeight.w500)),
    ]),
  );

  Widget _alertCard(IconData icon, Color color, String title, String message) => Container(
  ...
  Widget _exportRow(String type, Color color) => Row(children: [
    Expanded(child: OutlinedButton.icon(
      onPressed: () => _lancerExport(type),
      icon: Icon(Icons.picture_as_pdf_rounded, color: color, size: 15),
      label: Text('PDF', style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: 11)),
      style: OutlinedButton.styleFrom(
  ...
  Widget _simpleCard(IconData icon, Color iconColor, String title, String subtitle,
      Color subtitleColor, String trailing, Color trailingColor) =>
      Container(
        margin: const EdgeInsets.only(bottom: 8),
        padding: const EdgeInsets.all(12),
        ...
        child: Row(children: [
          ...
          Text(trailing, style: TextStyle(color: trailingColor, fontWeight: FontWeight.bold, fontSize: 12)),
        ]),
      );
  ...
  Widget _totalRow(String label, String value, Color color) => Container(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      Text(label.toUpperCase(), style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: 11, letterSpacing: 0.5)),
      Text('$value CFA', style: TextStyle(color: color, fontWeight: FontWeight.w900, fontSize: 14)),
    ]),
  );
  ...
  Widget _emptyCard(String msg) => Container(
    ...
    child: Center(child: Text(msg, style: TextStyle(color: Colors.grey.shade400, fontSize: 12))),
  );
}